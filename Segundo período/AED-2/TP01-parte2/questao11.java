...
{//Inicio classe questao01
    ...
    {//Inicio main
        ...
        Arq.openWrite("matrícula_shellsort.txt");//Abrindo arquivo de Log para escrita
...
{//Inicio classe Lista
    ...
    {//Inicio ordenarCodigo
        ...
        do{
            passo /= 3;
            for(int cor = 0; cor < passo; cor++)
                this.insercaoPorCor(cor, passo);
        }while(passo != 1);
    }//Fim ordenar codigo
     ...
     * @param passo Passo do shellsort
     */
    private void insercaoPorCor(int cor, int passo)
    {//Inicio insercaoPorCor
        Instituicao tmp;
        int j;
        for(int i = (passo + cor); i < numElementos; i += passo)
        {//Inicio for
            tmp = array[i].getClone();
            j = i - passo;
            while(j >= 0 && (array[j].getUF().compareToIgnoreCase(tmp.getUF()) > 0 || array[j].getUF().equals(tmp.getUF()) && array[j].getSigla().compareToIgnoreCase(tmp.getSigla()) > 0 ))
            {//Inicio while
