...
{
	int maior = array[0];
	#pragma omp parallel for shared(maior)
	for(i=0;i<n;i++)
	{
    	if(maior<array[i])
		{
...
int countingSort()
{
	...
	#pragma omp parallel for shared(count, tamCount)
	...
	//#pragma omp parallel for shared(array, count)
   	for(int i=0;i<n;i++)
	{
	...
	#pragma omp parallel for shared(array, ordenado)
   	for(i=0;i<n;i++)
	{
		array[i]=ordenado[i];
	}//fim for
