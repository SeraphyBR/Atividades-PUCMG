...
{//Inicio classe questao01
    ...
    {//Inicio main
        ...
        {//Inicio try
            ...
            listaInstituicao.ordenarPeriodico();//Ordena a lista com base no codigo das IES
        ...
        Arq.openWrite("matrícula_countingsort.txt");//Abrindo arquivo de Log para escrita
...
{//Inicio classe Lista
    ...
    /**
     * Retorna o maior periodico da Lista
     * @return O maior periodico
     */
    private int getMaiorPeriodico()
    {//Inicio getMaiorPeriodico
        int maior = array[0].getPeriodico();
        int tmp;
        for(int i = 0; i < numElementos; i++){
            tmp = array[i].getPeriodico();
            if(maior < tmp){
                maior = tmp;
            }
        }
    ...
    /**
     * Algoritmo de ordenacao Countingsort, com base no periodico.
     */
    public void ordenarPeriodico()
    {//Inicio ordenarPeriodico
        //Array para contar o numero de ocorrencias de cada elemento
        int[] count = new int[this.getMaiorPeriodico() + 1];
        ...
        for(int i = 0; i < numElementos; count[array[i].getPeriodico()]++, i++);

        //Agora, o count[i] contem o numero de elementos menores ou iguais a i
        for(int i = 1; i < count.length; count[i] += count[i-1], i++);

        //Ordenando
        for(int i = numElementos-1; i>= 0; ordenado[count[array[i].getPeriodico()] - 1] = array[i].getClone(), count[array[i].getPeriodico()]--, i--);
