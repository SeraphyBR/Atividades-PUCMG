...
{//Inicio classe questao09
    ...
    {//Inicio main
        ...
        {//Inicio try
            ...
            listaInstituicao.ordenarCodigoMantenedora(10);//Ordena a lista com base no codigo da Mantenedora das IES
            tempoFinal = System.currentTimeMillis();

            listaInstituicao.mostrar(10);
        }//Fim try
        catch(Exception exception){
            exception.printStackTrace();
        }

        Arq.openWrite("matrícula_insercao.txt");//Abrindo arquivo de Log para escrita
...
{//Inicio classe Lista
    ...
    {//Inicio mostrar
        try{
            this.mostrar(this.numElementos);
        } 
        catch(Exception exception){
    ...
    /**
     * Mostra os k primeiros elementos da lista separados por espacos
     * @param k Numero de elementos a serem mostrados
     * @throws Exception Se k for menor que 1, ou maior que o numero total de elementos
     */
    public void mostrar(int k) throws Exception
    {//Inicio mostrar
        if(k < 1 || k > numElementos) throw new Exception("Parametro possue valor invalido");
        for(int i = 0; i < k; i++){
            array[i].imprime();
        }
    }//Fim mostrar    
     ...
     */
    public void ordenarCodigoMantenedora()
    {//Inicior ordenarCodigo
        try{
            this.ordenarCodigoMantenedora(this.numElementos);
        }
        catch(Exception exception){
            System.err.println(exception);
        }
    ...
    /**
     * ALgoritmo de ordenacao parcial por insercao, com base
     * no codigo da Mantenedora da IES.
     * @param k Numero de elementos a serem ordenados.
     * @throws Exception Se k for menor que 1, ou maior que o numero total de elementos
     */
    public void ordenarCodigoMantenedora(int k) throws Exception
    {//Inicior ordenarCodigo
        Instituicao tmp;
        if(k < 1 || k > numElementos) throw new Exception("Parametro possue valor invalido");
        int j;
        ...
        {//Inicio for
            tmp = array[i].getClone();//movimentacao
            if(i > k - 1) j = k - 1;
            else j = i - 1;
            //Se os codigos forem iguais, ira ordenar pela sigla
            while(j >= 0 && array[j].getCodigoMantenedora() > tmp.getCodigoMantenedora() || 
                    ((array[j].getCodigoMantenedora() == tmp.getCodigoMantenedora()) && (tmp.getSigla().compareToIgnoreCase(array[j].getSigla()) < 0)))
            {//Inicio while
                array[j + 1] = array[j].getClone();//movimentacao
                j--;
    ...
    /**
     * Pesquisa uma instituicao na Lista, por meio do codigo da Mantenedora.
     * @param codigoM Codigo da mantenedora da IES a ser buscada.
     * @return Verdadeiro se encontrar na lista, falso caso contrario.
     */
    public boolean pesquisa(int codigoM)
    {//Inicio pesquisaBinaria
        ...
        {//Inicio while
            ...
            val = array[meio].getCodigoMantenedora();
            if(val == codigoM)
                encontrei = true;
            else if(val < codigoM) inicio = meio + 1;
            else if(val > codigoM) fim = meio - 1;
        }//Fim while
        return encontrei;
    }//Fim pesquisaBinaria 
}//Fim classe Lista
