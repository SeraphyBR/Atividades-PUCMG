...
{//Inicio classe questao01
    ...
    {//Inicio main
        ...
        {//Inicio try
            ...
            Lista listaInstituicao = new Lista();
        ...
        Arq.openWrite("matrícula_quicksort2.txt");//Abrindo arquivo de Log para escrita
...
{//Inicio classe Celula
    ...
    public CelulaDupla prox; // Aponta a proxima celula.
    public CelulaDupla ant; // Aponta a celula anterior.
    ...
    */
    public CelulaDupla(Instituicao i){
...
{//Inicio classe Lista
    ...
    {//Inicio inserirInicio
        CelulaDupla tmp = new CelulaDupla(instituicao);
        tmp.ant = primeiro;
    ...
    {//Inicio inserirFim
        ultimo.prox = new CelulaDupla(instituicao);
    ...
    {//Inicio swap
        ...
        CelulaDupla temp1 = primeiro.prox;
        CelulaDupla temp2 = primeiro.prox;
        for(int cont = 0; cont < posicao1; temp1 = temp1.prox, cont++ );
        for(int cont = 0; cont < posicao2; temp2 = temp2.prox, cont++ );

        Instituicao temp = temp1.elemento.getClone();
        temp1.elemento = temp2.elemento.getClone();
        temp2.elemento = temp;
        Conta.somaMovimentacoes(3);//Soma as 3 movimentacoes entre elementos do array do swap.
    }//Fim swap

    /**
     * Metodo para obter uma instituicao da Lista Flexivel.
     * @param posicao Posicao da lista 0 a (numElementos - 1).
     * @throws Exception Se for uma posicao inválida.
     */
    public Instituicao getElemento(int posicao) throws Exception 
    {//Inicio getElemento
        if(posicao >= numElementos || posicao < 0){
            throw new Exception("Posicao invalida!");
        }
        
        Instituicao elemento = null;
        if(posicao == 0) elemento = primeiro.prox.elemento;
        else if (posicao == numElementos - 1) elemento = ultimo.elemento;
        else{
            CelulaDupla temp = primeiro.prox;
            for(int cont = 0; cont < posicao; temp = temp.prox, cont++);
     ...
     * @throws Exception Se tiver menos de 2 elementos
     */
     ...
     * Algoritmo de ordenacao Quicksort,
     * com base na ordem lexicografica dos Nomes das IES.
     * @param esq Inicio da Lista a ser ordenada.
     * @param dir Fim da Lista a ser ordenada.
     * @throws Exception Se getElemento() receber posicao invalida. 
     */
    private void ordenarNome(int esq, int dir) throws Exception
    {//Inicio ordenarSigla
        int i = esq, j = dir;
        String pivo = this.getElemento((dir + esq)/2).getNome();
        while(i <= j)
        {//Inicio while
            //Compara se a sigla na posicao i e lexicograficamente menor que o pivo.
            while(this.getElemento(i).getNome().compareTo(pivo) < 0){
                i++;
                Conta.somaComparacoes();
            }

            //Compara se a sigla na posicao j e lexicograficamente maior que o pivo.
            while(this.getElemento(j).getNome().compareTo(pivo) > 0){
                j--;
                Conta.somaComparacoes();
            }
     ...
     * @throws Exception
     */
    public boolean pesquisa(String nome) throws Exception
    {//Inicio pesquisaBinaria
        ...
        while(inicio <= fim && !encontrei){
            ...
            compareResult = this.getElemento(meio).getNome().compareTo(nome);
            Conta.somaComparacoes();
            if(compareResult == 0)
                encontrei = true;
