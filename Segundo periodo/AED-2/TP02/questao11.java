...
{//Inicio classe questao01
    ...
    {//Inicio main
        ...
        {//Inicio try
            ...
            listaInstituicao.ordenarNome(10);//Ordena a lista com base no codigo das IES
...
{//Inicio classe Lista
    ...
    public void ordenarNome(){
        try{
            this.ordenarNome(this.numElementos);
        }
        catch(Exception exception){
            System.err.println(exception);
        }
     ...
     * @throws Exception Se k for menor que 1, ou maior que o total de elementos.
     */
    public void ordenarNome(int k) throws Exception{
        int fim = numElementos - 1;
        if(k < 1 || k > this.numElementos) throw new Exception("Parametro possue valor invalido!");
        if(fim > 0) ordenarNome(0, fim, k);
     ...
     * @param k Numero de elementos a serem ordenados
     */
    private void ordenarNome(int esq, int dir, int k) throws Exception
    {//Inicio ordenarSigla
        int i = esq, j = dir;
        ...
        if((j - esq) >= k){
            //Se o numero de elementos a esquerda for maior ou igual a k elementos
            //Pode-se descartar a chamada recursiva a direita.
            if(esq < j) this.ordenarNome(esq, j, k);
        }
        else{
            if(esq < j) this.ordenarNome(esq, j, k);
            if(i < dir) this.ordenarNome(i, dir, k);
        }
    }//Fim ordenarSigla  
    /**
